# 1
lista_reales = []
tupla_menus = ("Llegir dades","Calcular el recorregut","Calcular la mitja","Calcular la variància","Salir")
...
def menu(tupla_menus):
    string_menu = "\n"

    for i in range(len(tupla_menus)):
        string_menu += str(i+1)+")"+ tupla_menus[i]+"\n"
    print(string_menu)
    ...
    while True:
        ...
        if not opcion.isdigit():
            print("la opcion ha de ser numerica")
        
        if not int(opcion) in range(len(tupla_menus) + 1):
